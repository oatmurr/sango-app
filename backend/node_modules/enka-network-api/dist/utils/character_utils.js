...
function getCharactersById(id, enka) {
    ...
    if (data.has("candSkillDepotIds") && (data.get("candSkillDepotIds").mapArray((_, p) => p.getAsNumber())).length > 0) {
        return data.get("candSkillDepotIds").filterArray((_, skillDepotId) => hasEnergySkill(skillDepotId.getAsNumber(), enka)).map(([, skillDepotId]) => new CharacterData_1.CharacterData(data.getAsJsonObject(), enka, skillDepotId.getAsNumber()));
    }
    else {
        if (!hasEnergySkill(data.getAsNumber("skillDepotId"), enka))
            return [];
        return [new CharacterData_1.CharacterData(data.getAsJsonObject(), enka)];
    }
}
