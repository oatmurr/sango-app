...
class CharacterData {
    constructor(data, enka, candSkillDepotId) {
        ...
        this.nameCard = nameCardId ? Material_1.Material.getMaterialById(nameCardId, enka) : null;
        ...
        this.costumes = this._costumeData.map(c => new Costume_1.Costume(c, enka));
        ...
        this.elementalBurst = hasElement ? ElementalBurst_1.ElementalBurst.getById(skillData.getAsNumber("energySkill"), enka) : null;
        ...
        this.skills = _skills;
        this.elementalSkill = (_f = _skills.find(s => s instanceof ElementalSkill_1.ElementalSkill)) !== null && _f !== void 0 ? _f : null;
        this.normalAttack = _skills.find(s => s instanceof NormalAttack_1.NormalAttack);
        this.passiveTalents = skillData.get("inherentProudSkillOpens").filterArray((_, p) => p.has("proudSkillGroupId")).map(([, p]) => PassiveTalent_1.PassiveTalent.getById(p.getAsNumber("proudSkillGroupId") * 100 + 1, enka)); // Number(`${p.proudSkillGroupId}01`)
        this.constellations = skillData.get("talents").mapArray((_, p) => p.getAsNumber()).filter(cId => cId !== 0).map(cId => Constellation_1.Constellation.getById(cId, enka));
        ...
        let details;
        try {
            details = CharacterDetails_1.CharacterDetails.getByCharacterId(this.id, this.isArchon, this.enka);
        }
        catch (e) {
            details = null;
        }
    ...
    getAscensionData(ascension) {
        return CharacterAscension_1.CharacterAscension.getById(new config_file_js_1.JsonReader(this._data).getAsNumber("avatarPromoteId"), ascension, this.enka);
    }
    /**
     * @param ascension ascension level between 0 and 6
    ...
    static getById(id, enka, candSkillDepotId) {
        ...
        if (!json)
            throw new AssetsNotFoundError_1.AssetsNotFoundError("Character", id);
